...
def get_fight_stats(event_id, fight_id):
    ...
    # Если бой завершён, но все значения нулевые – возвращаем специальный флаг, чтобы пропустить
    if finished and not round_data:
        return {"finished": True, "skip": True, "names": names, "photos": photos, "winner_idx": winner_idx}
...
def main():
    ...
    # Обработка случая "завершён, но без данных"
    if data.get("skip"):
        print(f"Бой {current_fight_id} завершён, но статистика пуста. Пропускаем.")
        if os.path.exists(MSG_ID_FILE):
            os.remove(MSG_ID_FILE)
        current_index += 1
